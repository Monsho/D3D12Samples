...
public:
	...
	bool Execute() override
	{
		...
		// TopASの更新
		{
			topAsTimestamp_[frameIndex].Reset();
			topAsTimestamp_[frameIndex].Query(&asCmdList);

			std::array<sl12::TopInstanceDesc, 2> topInstances;
			DirectX::XMFLOAT4X4 mtx0;
		...
		// Z pre pass
		{
			...
			RenderMesh(&moveMesh_, &moveMeshCBVs_[frameIndex]);
		}
	...
	void Finalize() override
	{
		...
		for (auto&& v : moveMeshCBVs_) v.Destroy();
		for (auto&& v : moveMeshCBs_) v.Destroy();
		glbMeshCBV_.Destroy();
		glbMeshCB_.Destroy();
...
private:
	...
	bool CreateGeometry()
	{
		...
		if (!moveMesh_.Initialize(&device_, &utilCmdList_, "data/", "suzanne.glb"))
		{
			return false;
		}
	...
	bool CreateAccelerationStructure()
	{
		...
		if (!CreateBottomASFromGlbMesh(&cmdList, &moveMesh_, &moveBottomAS_))
		{
			return false;
		}
	...
	bool CreateMeshCB()
	{
		...
		for (int i = 0; i < kBufferCount; i++)
		{
			if (!moveMeshCBs_[i].Initialize(&device_, sizeof(MeshCB), 0, sl12::BufferUsage::ConstantBuffer, true, false))
			{
				return false;
			}
			else
	...
	bool CreateShaderTable()
	{
		...
		for (int i = 0; i < moveMesh_.GetSubmeshCount(); i++)
		{
			auto submesh = moveMesh_.GetSubmesh(i);
			auto material = moveMesh_.GetMaterial(submesh->GetMaterialIndex());
			auto texView = moveMesh_.GetTextureView(material->GetTexBaseColorIndex());
			auto texNView = moveMesh_.GetTextureView(material->GetTexNormalIndex());
		...
		if (!GenShaderTable(hitGroupShaderIdentifier, 1, hitGroupTable_, glbMesh_.GetSubmeshCount() + moveMesh_.GetSubmeshCount(), hitGroupIndices.data()))
		{
			return false;
		}
...
private:
	...
	sl12::GlbMesh			glbMesh_, moveMesh_;
	sl12::Sampler			imageSampler_;

	sl12::BottomAccelerationStructure	glbBottomAS_, moveBottomAS_;
	...
	sl12::Buffer				moveMeshCBs_[kBufferCount];
	sl12::ConstantBufferView	moveMeshCBVs_[kBufferCount];
	...
	float					meshTime_ = 0.0f;
	bool					isSuzanneStop_ = false;
