...
void RayGenerator()
{
	...
	// シャドウ用レイの生成
	float2 ham = Hammersley2D(cbTime.loopCount % MaxSample, MaxSample);
	...
	// 結果の生成
	float result = shadowPay.visible;

	// 前回までの結果とブレンド
	float prev = RenderTarget[index] * float(cbTime.loopCount);
	RenderTarget[index] = (prev + result) / float(cbTime.loopCount + 1);
}
