...
public:
	...
	bool Initialize() override
	{
		...
		}
		{
			D3D12_DESCRIPTOR_RANGE ranges[] = {
				...
				{ D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND },		// DestLightMap
			...
			if (!lightBakeSystem_.globalRootSig.Initialize(&device_, sigDesc))
			{
				return false;
			}
		}
		{
			...
			desc.numParameters = ARRAYSIZE(params);
			desc.flags = D3D12_ROOT_SIGNATURE_FLAG_LOCAL_ROOT_SIGNATURE;
			if (!lightBakeSystem_.localRootSig.Initialize(&device_, desc))
			{
				return false;
			}
		}
		...
		}
		{
			sl12::RootParameter params[] = {
				...
				sl12::RootParameter(sl12::RootParameterType::ShaderResource, sl12::ShaderVisibility::Pixel, 2),
				sl12::RootParameter(sl12::RootParameterType::Sampler, sl12::ShaderVisibility::Pixel, 2),
			};
			sl12::RootSignatureDesc desc;
			desc.pParameters = params;
			desc.numParameters = ARRAYSIZE(params);
		...
		}
		{
			...
			if (!displayAtlasRootSig_.Initialize(&device_, sigDesc))
			{
				return false;
			}
		}
		{
			...
			if (!bakePosNmlRootSig_.Initialize(&device_, sigDesc))
			{
				return false;
			}
		}
		{
			...
			if (!denoiseRootSig_.Initialize(&device_, desc))
			{
				return false;
			}
		}
	...
	bool Execute() override
	{
		...
		if (bakeLoopCount_ < MaxSample)
		{
			// ライトベイク用のレイトレース
			// グローバルルートシグネチャを設定
			d3dCmdList->SetComputeRootSignature(lightBakeSystem_.globalRootSig.GetRootSignature());

			// グローバル設定のシェーダリソースを設定する
			d3dCmdList->SetComputeRootDescriptorTable(0, randomBufferSRV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetComputeRootDescriptorTable(1, posBufferTextureSRV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetComputeRootDescriptorTable(2, nmlBufferTextureSRV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetComputeRootDescriptorTable(3, lightMapTextureUAV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetComputeRootDescriptorTable(4, seedBufferUAV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetComputeRootDescriptorTable(5, sceneCBVs_[frameIndex].GetDesc()->GetGpuHandle());
			...
			d3dCmdList->SetGraphicsRootSignature(denoiseRootSig_.GetRootSignature());

			d3dCmdList->SetGraphicsRootDescriptorTable(0, lightMapTextureSRV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetGraphicsRootDescriptorTable(1, posBufferTextureSRV_.GetDesc()->GetGpuHandle());
			d3dCmdList->SetGraphicsRootDescriptorTable(2, nmlBufferTextureSRV_.GetDesc()->GetGpuHandle());
		...
		// lighting pass
		{
			...
			if (enableDenoise_)
			{
				d3dCmdList->SetGraphicsRootDescriptorTable(4, denoiseTextureSRV_.GetDesc()->GetGpuHandle());
			}
			else
			{
				d3dCmdList->SetGraphicsRootDescriptorTable(4, lightMapTextureSRV_.GetDesc()->GetGpuHandle());
			}
			d3dCmdList->SetGraphicsRootDescriptorTable(5, linearWrapSampler_.GetDesc()->GetGpuHandle());
